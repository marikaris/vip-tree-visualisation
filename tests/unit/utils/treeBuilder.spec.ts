...
import { TreeEdgesObj } from '@/types/TreeEdges'
...
describe('retrieveTreeFromFile', () => {
  ...
  it('should generate edges', () => {
    ...
    const expected = [
      { from: 'boolNode', to: 'exit_f', label: 'True' },
      { from: 'boolNode', to: 'existsNode', label: 'False\nMissing' },
      { from: 'existsNode', to: 'categorical_field', label: 'True\nFalse' },
      { from: 'categoricalField', to: 'exit_t', label: 'P\nLP\nMissing\nDefault' },
      { from: 'categoricalField', to: 'exit_f', label: 'LB\nB' }
    ]
    expect(outcome).toStrictEqual(expected)
  })
})
